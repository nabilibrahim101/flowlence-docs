...
!!! question "Challenge 3 · The full safety logic"
    Add the water-level check: **IF moisture > 3000 AND water-level > 500 → pump on**. If the tank is empty (water-level low), don't run the pump even if the soil is dry, or it would burn out. This is exactly the build you'll complete next.
